...
pipeline{
    ...
    stage('Deploy docker image in DEV machine using ansible'){
	    steps{
		    ansiblePlaybook credentialsId: 'ec2user', extras: '--extra-vars "imageversion=${BUILD_NUMBER}"', installation: 'ansible', inventory: 'Project1/dev-inv', playbook: 'Project1/ansible-deploy.yml'
	    }
    }
   }
}
